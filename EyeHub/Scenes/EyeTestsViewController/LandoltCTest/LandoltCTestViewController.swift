//
//  LandoltCTestViewController.swift
//  EyeHub
//
...
class LandoltCTestViewController: UIViewController {
    let testData: [LandoltCTestData] = LandoltCTestData.allCases
...
extension LandoltCTestViewController: NavigationBarDelegate {
    func navigationBackButtonDidTap(_ navigation: NavigationBar) {
        navigationController?.popViewController(animated: true)
    }
}

extension LandoltCTestViewController: CollectionViewBottomSheetDelegate {
...
private extension LandoltCTestViewController {
    func commonInit() {
        setUpUI()
    ...
    func navigateToSummary() {
        let newViewController = EyeTestsSummaryViewController(testType: .landoltC, testResult: testResults)
    ...
    func createQuestion() {
        ...
        for rotation in wrongRotations {
            questionArr.append(EandCModel(text: CollectionViewTestType.landoltC, transform: rotation))
        }
        
        let randomIndex = Int.random(in: 0..<wrongRotations.count + 1)
        correctAnswer = randomIndex
        
        questionArr.insert(EandCModel(text: CollectionViewTestType.landoltC, transform: testRotation), at: randomIndex)
